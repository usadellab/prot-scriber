...
mcc <- function(pred, ref, univ.words) {
    ...
    true.neg <- length(setdiff(univ.words, union(pred, 
        ref)))
...
#' @export
matthewsCorrelationCoefficient <- function(pred, ref, 
    univ.words, prot.id, pred.to.string.funk = getOption("fScore.pred.to.string.funk", 
        function(wrds) {
            paste(wrds, collapse = " ")
        })) {
    ...
    if (length(ref) == 0 || all(is.na(ref))) {
        ...
        data.frame(Protein.ID = prot.id, HRD = hrd, 
            ...
            ref.words = length(ref), univ.ref.words = length(intersect(univ.words, 
                ref)), MCC = NA, MCC.relative = NA, 
        ...
        m.c.c.best <- mcc(intersect(ref, univ.words), 
            ref, univ.words)
        if (m.c.c > m.c.c.best) {
            warning("MCC.best.possible (", m.c.c.best, 
        ...
        data.frame(Protein.ID = prot.id, HRD = hrd, 
            n.words = length(pred), univ.words = length(univ.words), 
            ref.words = length(ref), univ.ref.words = length(intersect(univ.words, 
                ref)), MCC = m.c.c, MCC.relative = m.c.c.rel, 
...
testMatthewsCorrelationCoefficient <- function() {
    ...
    t3 <- identical(matthewsCorrelationCoefficient(p3, 
        r3, u3, "P3")$MCC, -0.5)
    t4.res <- matthewsCorrelationCoefficient(p3, NULL, 
        NULL, "P3")
    t4 <- is.na(t4.res$MCC)
    p5 <- NA
    r5 <- c("kinase", "domain")
    t5.res <- matthewsCorrelationCoefficient(p5, r5, 
        r5, "P5")
    t5 <- identical(t5.res$MCC, -1)
    p6 <- character(0)
    r6 <- c("kinase", "domain")
    t6.res <- matthewsCorrelationCoefficient(p6, r6, 
        r6, "P6")
    t6 <- identical(t6.res$MCC, -1)
    all(t1, t2, t3, t4, t5)
}
...
#' @export
fScore <- function(pred, ref, univ.words, prot.id, 
    beta = getOption("fScore.beta", 1), pred.to.string.funk = getOption("fScore.pred.to.string.funk", 
        function(wrds) {
            paste(wrds, collapse = " ")
        })) {
...
wordUniverse <- function(prot.id, sssr, to.lower = getOption("wordUniverse.to.lower", 
    ...
    univ.words <- unique(unlist(lapply(sssr, function(seq.sim.tbl) {
        ...
        if (length(hits.ind) > 0) {
            unlist(lapply(seq.sim.tbl[hits.ind, ]$HRD, 
                wordSet))
        } else {
            character(0)
        }
...
annotateProteinsAndEvaluatePerformance <- function(sssr, 
    ...
    do.call(rbind, mclapply(prot.ids, function(qseqid) {
        tryCatch({
            ...
            rbind(bestBlastHrds(qseqid, sssr, ref.words, 
                univ.words), do.call(rbind, lapply(names(prot.scriber.score.funks), 
...
measurePredictionsPerformance <- function(query.id, 
    ...
    if (query.id %in% names(refs)) {
        ...
        #' TODO:
        #' Intersect the result of the below 'union' with the "gold standard"
        #' i.e. 'refs'. Write a function that generates this "fair" word-universe.
        ...
        if (!is.null(ahrd.res)) {
            ...
            query.ahrd.tbl <- data.frame(HRD = query.ahrd.hrd, 
                n.words = ahrd.n.words, stringsAsFactors = FALSE)
            ahrd.performance <- bestProtScriberPhrases(query.id, 
