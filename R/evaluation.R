...
mcc <- function(pred, ref, univ.words) {
    true.pos <- length(intersect(pred, 
        ref))
    true.neg <- length(setdiff(univ.words, 
        union(pred, ref)))
    false.pos <- length(setdiff(pred, 
        ref))
    false.neg <- length(setdiff(ref, 
        pred))
    mcc.denominator <- sqrt((true.pos + 
        false.pos) * (true.pos + 
        false.neg) * (true.neg + 
        false.pos) * (true.neg + 
        false.neg))
    if ((true.pos + false.pos) == 
        0 || (true.pos + false.neg) == 
        0 || (true.neg + false.pos) == 
        0 || (true.neg + false.neg) == 
        0) {
        mcc.denominator <- 1
    }
    m.c.c <- if (identical(pred, 
        ref)) {
        1
    } else {
        tp.tn <- if (length(setdiff(univ.words, 
            ref)) == 0) {
...
#' @export
matthewsCorrelationCoefficient <- function(pred, 
    ref, univ.words, prot.id, pred.to.string.funk = getOption("fScore.pred.to.string.funk", 
        function(wrds) {
            paste(wrds, collapse = " ")
        })) {
    ...
    if (length(ref) == 0 || is.na(ref)) {
        ...
        data.frame(Protein.ID = prot.id, 
            HRD = hrd, n.words = length(pred), 
            univ.words = length(univ.words), 
            ref.words = length(ref), 
            univ.ref.words = length(intersect(univ.words, 
                ref)), MCC = NA, 
            MCC.relative = NA, 
            MCC.best.possible = NA, 
            stringsAsFactors = FALSE)
    } else {
        m.c.c <- mcc(pred, ref, 
            univ.words)
        #' Best MCC achievable with the seq-sim-search results words:
        m.c.c.best <- mcc(intersect(ref, 
            univ.words), ref, univ.words)
        if (m.c.c > m.c.c.best) {
            warning("MCC.best.possible (", 
                m.c.c.best, ") is smaller than MCC (", 
                m.c.c, ")! This situation should NEVER arise.")
        }
        #' Fraction of the best MCC achievable actually reached:
        m.c.c.rel <- 1 - (m.c.c.best - 
            m.c.c)/(m.c.c.best - 
            -1)
        data.frame(Protein.ID = prot.id, 
            HRD = hrd, n.words = length(pred), 
...
testMatthewsCorrelationCoefficient <- function() {
    ...
    r1 <- p1
    u1 <- p1
    t1 <- identical(matthewsCorrelationCoefficient(p1, 
        r1, u1, "P1")$MCC, 1)
    p2 <- c("Hello", "World")
    r2 <- c("Not", "a", "single", 
        "word")
    u2 <- union(p2, r2)
    t2 <- identical(matthewsCorrelationCoefficient(p2, 
        r2, u2, "P2")$MCC, -1)
    p3 <- c("Hello", "World", "Hola", 
        "Mundo")
    r3 <- c("Some", "words", "Hello", 
        "World")
    u3 <- union(p3, r3)
    t3 <- identical(matthewsCorrelationCoefficient(p3, 
        r3, u3, "P3")$MCC, -0.5)
    t4.res <- matthewsCorrelationCoefficient(p3, 
        NULL, NULL, "P3")
    t4 <- is.na(t4.res$MCC)
    all(t1, t2, t3, t4)
}
...
#' @export
fScoreCalculator <- function(pred, 
    ref, beta = getOption("fScore.beta", 
        1)) {
    true.pos <- intersect(pred, 
        ref)
    precision <- if (length(pred) > 
        0) {
    ...
    f.c <- if ((beta * precision + 
        recall) == 0) {
        0
    } else {
        (1 + beta^2) * (precision * 
            recall)/(beta * precision + 
            recall)
    }
    data.frame(F.Score = f.c, precision = precision, 
        recall = recall, fScore.beta = beta, 
        stringsAsFactors = FALSE)
}
...
#' @export
fScore <- function(pred, ref, univ.words, 
    prot.id, beta = getOption("fScore.beta", 
    ...
    if (length(ref) == 0 || is.na(ref)) {
        ...
        data.frame(Protein.ID = prot.id, 
            ...
            F.Score = NA, F.Score.relative = NA, 
            F.Score.best.possible = NA, 
            fScore.beta = beta, 
            stringsAsFactors = FALSE)
    } else {
        f.score.df <- fScoreCalculator(pred, 
            ref, beta = beta)
        #' Best possibly achievable F.Score given the seq-sim-search results'
        #' words:
        f.score.best <- fScoreCalculator(intersect(ref, 
            univ.words), ref, beta = beta)$F.Score
        #' How much of the best possible was achieved?
        f.score.rel <- if (f.score.best == 
            0) {
...
testFScore <- function() {
    ...
    r1 <- p1
    u1 <- p1
    t1 <- identical(fScore(p1, 
        r1, u1, "P1", beta = beta)$F.Score, 
        1)
    p2 <- c("Hello", "World")
    r2 <- c("Not", "a", "single", 
        "word")
    u2 <- p2
    t2 <- identical(fScore(p2, 
        r2, u2, "P2", beta = beta)$F.Score, 
        0)
    p3 <- c("Hello", "World", "Hola", 
        "Mundo")
    r3 <- c("Some", "words", "Hello", 
        "World")
    u3 <- p3
    t3 <- identical(fScore(p3, 
        r3, u3, "P3", beta = beta)$F.Score, 
        0.5)
    t4.res <- fScore(p3, NULL, 
        u3, "P3", beta = beta)
    t4 <- is.na(t4.res$F.Score) && 
        is.na(t4.res$precision) && 
        is.na(t4.res$recall)
    all(t1, t2, t3, t4)
}
...
#' @export
bestBlastHrds <- function(prot.id, 
    sssr, hrd.ref, univ.words, 
    ...
    for (ref.db.name in names(sssr)) {
        ...
        if (prot.id %in% seq.sim.search.tbl$qseqid) {
            ...
            q.hits$HRD.nchar <- as.integer(lapply(q.hits$HRD, 
                nchar))
            q.hits.best <- q.hits[order(q.hits$bitscore, 
                q.hits$HRD.nchar, 
                decreasing = TRUE), 
                ][1, ]
            ...
            bb.hrd.word.set <- wordSet(bb.hrd, 
        ...
        bb.hrd.f.score.df <- fScore(bb.hrd.word.set, 
            hrd.ref, univ.words, 
            prot.id)
        bb.hrd.mcc.df <- matthewsCorrelationCoefficient(bb.hrd.word.set, 
            hrd.ref, univ.words, 
            prot.id)[, c("Protein.ID", 
            "MCC", "MCC.relative", 
            "MCC.best.possible", 
            "univ.words", "ref.words", 
            "univ.ref.words")]
        bb.hrd.eval.df <- merge(bb.hrd.f.score.df, 
            bb.hrd.mcc.df, by = "Protein.ID")
        bb.hrd.eval.df$Method <- paste0("BB.", 
            ref.db.name)
        bb.hrd.eval.df$Method.Score <- method.score
        bb.hrd.lst[[length(bb.hrd.lst) + 
            1]] <- bb.hrd.eval.df
    }
...
#' @export
bestProtScriberPhrases <- function(prot.id, 
    phrases.stats, prot.scriber.score.col, 
    hrd.ref, univ.words) {
    if (nrow(phrases.stats) > 0) {
        ps.best <- phrases.stats[order(phrases.stats[, 
        ...
        #' to 'NULL':
        ps.best.word.set <- wordSet(ps.best$HRD, 
            blacklist.regexs = NULL)
        ps.best.f.score.df <- fScore(ps.best.word.set, 
            hrd.ref, univ.words, 
            prot.id)
        ...
        ps.best.eval.df <- merge(ps.best.f.score.df, 
            ps.best.mcc.df, by = "Protein.ID")
        ps.best.eval.df$Method <- prot.scriber.score.col
        ps.best.eval.df$Method.Score <- ps.best[, 
            prot.scriber.score.col]
        ps.best.eval.df
    }
}
...
#' @export
wordUniverse <- function(prot.id, 
    sssr, to.lower = getOption("wordUniverse.to.lower", 
        TRUE)) {
    univ.words <- unique(unlist(lapply(sssr, 
        function(seq.sim.tbl) {
            hits.ind <- which(seq.sim.tbl$qseqid == 
                prot.id)
            if (length(hits.ind) > 
                0) {
                unlist(lapply(seq.sim.tbl[hits.ind, 
                  ]$HRD, wordSet))
            } else {
                character(0)
            }
...
#' Protein.ID, HRD, precision, recall, F.Score, beta, Method, Method.Score
#' @export
annotateProteinsAndEvaluatePerformance <- function(sssr, 
    ref.word.sets, prot.scriber.score.funks = getOption("statsOfPhrasesForQuery.score.funks", 
        list(centered.inverse.inf.cntnt.mean = list(word.score.funk = function(x) {
            centeredWordScores(x, 
                level.funk = mean)
        }, phrase.score.funk = sumWordScores), 
            centered.inverse.inf.cntnt.median = list(word.score.funk = function(x) {
                centeredWordScores(x, 
    ...
    do.call(rbind, mclapply(prot.ids, 
        function(qseqid) {
            tryCatch({
                q.phrases <- phrasesForQuery(qseqid, 
                  sssr)
                ...
                univ.words <- wordUniverse(qseqid, 
                  sssr)
                rbind(bestBlastHrds(qseqid, 
                  sssr, ref.words, 
                  univ.words), 
                  do.call(rbind, 
                    lapply(names(prot.scriber.score.funks), 
                      function(ps.score.method) {
                        bestProtScriberPhrases(qseqid, 
                          q.phrases.stats, 
                          ps.score.method, 
                          ref.words, 
                          univ.words)
...
annotateProteinsAndEvaluatePerformance.MultiRegion <- function(sssr, 
    ref.word.sets, alignment.regions.df, 
    ...
    do.call(rbind, mclapply(prot.ids, 
        function(qseqid) {
            tryCatch({
                ref.words <- ref.word.sets[[tolower(qseqid)]]
                qseqid.sssr.regions <- sssrForRegions(qseqid, 
                  alignment.regions.df, 
                  sssr)
                qseqid.region.hrds <- do.call(rbind, 
                  lapply(qseqid.sssr.regions, 
                    function(sssr.i) {
                      q.phrases <- phrasesForQuery(qseqid, 
                        sssr.i)
                      q.phrases.stats <- statsOfPhrasesForQuery(qseqid, 
...
#' @export
measurePredictionsPerformance <- function(query.id, 
    sssr, ps.res, refs, ps.na.hrd = getOption("ps.na.hrd", 
        "unknown protein")) {
    if (query.id %in% names(refs)) {
        ...
        #' the word universe:
        ps.words <- if (query.id %in% 
            ps.res$Annotee.Identifier) {
            wordSet(ps.res[[query.id, 
                "Human.Readable.Description"]], 
                blacklist.regexs = NULL)
        } else {
            c()
        }
        univ.words <- union(ps.words, 
            wordUniverse(query.id, 
                sssr))
        best.blast.performance <- bestBlastHrds(query.id, 
            sssr, query.ref, univ.words)
        
        #' prot-scriber performance:
        query.ps.hrd <- if (query.id %in% 
            ps.res$Annotee.Identifier) {
            ps.hrd <- ps.res[[query.id, 
                "Human.Readable.Description"]]
            if (ps.hrd == ps.na.hrd) {
        ...
        ps.n.words <- if (is.na(query.ps.hrd)) {
            ...
            length(strsplit(query.ps.hrd, 
                " ")[[1]])
        }
        query.ps.tbl <- data.frame(HRD = query.ps.hrd, 
            n.words = ps.n.words, 
            stringsAsFactors = FALSE)
        prot.scriber.performance <- bestProtScriberPhrases(query.id, 
            query.ps.tbl, 2, query.ref, 
            univ.words)
        ...
        #' Result:
        rbind(best.blast.performance, 
            prot.scriber.performance)
    } else {
        message("WARNING: Could not find query.id '", 
