...
option_list <- list(
  ...
  make_option(
    c("-f", "--use-fair-references"),
    type = "logical",
    default = TRUE,
    help = "If set the evaluation intersects all reference words (Pfam-A and Mercator/Mapman Bin) with words appearing in the Blast Hit Descriptions in order to be 'fair' when calculating false negatives and true negatives. We justify this approach as 'fair' because Best Blast and prot-scriber cannot include words in their HRD that do not appear in the input Blast Hit Descriptions (options '-t' and '-s'), even though they might appear in Pfam-A and Mapman Bin Descriptions. Default of this options is TRUE.",
    metavar = "logical"
  )
)
...
queries.ahrd <- if (!is.null(script.args$ahrd)) {
  ahrd.tbl <- read.table(script.args$ahrd,
    sep = "\t",
...
queries.ref <- mergeMercatorAndPfamAReferences(
...
query.ids <- intersect(names(queries.ref), unique(c(
  blast.sprot$qseqid,
...
queries.performance <- do.call(rbind, mclapply(
  query.ids,
  function(q.id) {
    measurePredictionsPerformance(q.id, queries.sssr,
      queries.prot.scriber, queries.ref,
      ahrd.res = queries.ahrd, use.fair.references = script.args$`use-fair-references`
    )
  }
))
...
message(
  "Written performance scores into '", script.args$`result-table`,
  "'."
)
...
perf.diff.sign.df <- do.call(rbind, lapply(c(
  "F.Score",
...
hyp.test.res.file <- sub(
  "\\.[^.]+$", "_hypothesis_tests.txt",
...
message(
  "Written significance tests of differences in performance scores between compared annotators into '",
  hyp.test.res.file, "'."
)
...
for (scr in plot.scores) {
  ...
  plot.df <- queries.performance.san[i, c(
    "Method",
    scr
  )]
...
message(
  "Generated boxplots of the score distributions in files '",
  plot.filename, "_*.pdf'."
)
