...
fn main() {
    let matches = App::new("Prot-Scribr")
        ...
        .arg(
            ...
            .long("seq-sim-table")
            .multiple(true)
            .about("File in which to find sequence similarity search results in tabular format. Use e.g. Blast or Diamond to produce them. Required columns are: 'qacc qlen qstart qend sacc slen sstart send bitscore stitle'. If given in different order than shown here use the --header arg."),
        )
        .arg(
