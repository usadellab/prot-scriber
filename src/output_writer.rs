...
pub fn write_output_table( file_path: String, human_readable_descriptions: HashMap<String, String> ) -> std::io::Result<()>{
...
mod tests {
    ...
    fn writer_test() {
        ...
        human_readable_descriptions.insert("Seq-Family-1".to_string(), "alien devouring protein".to_string());
        human_readable_descriptions.insert("Protein-123".to_string(), "human devouring protein".to_string());
        assert_eq!(write_output_table("./target/result.txt".to_string(), human_readable_descriptions).is_ok(), true);
    }
}
